...
### Load in meta data
env_fact <- read.table(file = "https://raw.githubusercontent.com/gerard-ricardo/data/master/postset%20treat%20mil", 
                       header = TRUE, dec = ",", na.strings = c("", ".", "NA"))
...
# Split data by individual ID
data1_s <- subset(data1_l, id %in% 'm007Y(II)1')  #remove factor treatment level. Use '%in%' to keep. 
...
# Plot original data with fitted model
p1 <- ggplot(data1_s) + 
